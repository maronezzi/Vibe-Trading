"""
backtest_agi_v12.py — Backtest com config AGI 17h v12.

Estratégias por timeframe:
  WDO M5  → SMART_EMA (pullback) — EMA 9/21, ADX>25, RSI filter
  WDO M15 → SMART_EMA (strong trend) — EMA 9/21, ADX>25, agressivo
  WDO M30 → MACD_MOMENTUM — MACD 12/26/9, EMA trend, ADX>15
  WDO H1  → BOLLINGER_REVERSION — BB 20/2σ, RSI <30/>70

  WIN M5  → EMA_PULLBACK — EMA 9/21, ADX>20, pullback 0.15%
  WIN M15 → EMA_PULLBACK — mesmo com mais barras
  WIN M30 → MACD_MOMENTUM — MACD 12/26/9, ADX>15
  WIN H1  → STRONG_TREND_WIDE — EMA 20/50, ADX>20, RSI loose
...
# ─── Strategy: STRONG_TREND (WIN M5/M15) ────────────────────────────────────
...
# ─── Strategy: MACD_MOMENTUM (M30 — WDO e WIN) ─────────────────────────────

def check_macd_momentum(price, atr_val, ema_fast_val, ema_slow_val, adx_val, plus_di, minus_di, rsi_val, macd_hist, prev_hist, prev2_hist, params):
    """
    MACD Momentum para M30.
    - EMA trend como filtro
    - MACD histogram cruzando OU momentum crescente
    - ADX mínimo para confirmar tendência
...
# ─── Strategy: BOLLINGER_REVERSION (H1 — WDO) ──────────────────────────────

def check_bollinger_reversion(price, atr_val, rsi_val, bb_upper, bb_mid, bb_lower, params):
    """
    Bollinger Reversion para H1 WDO.
    - Compra quando preço toca banda inferior + RSI oversold
    - Vende quando preço toca banda superior + RSI overbought
    - Mean-reversion: espera reversão à média
    """
...
# ─── Strategy: STRONG_TREND_WIDE (H1 — WIN) ────────────────────────────────

def check_strong_trend_wide(price, atr_val, ema_fast_val, ema_slow_val, adx_val, plus_di, minus_di, rsi_val, params):
    """
    Strong Trend com parâmetros largos para H1 WIN.
    - EMA 20/50 (mais lenta, captura macro-tendência)
    - ADX > 20 (mais permissivo no H1)
    - RSI loose (só filtra extremos)
...
def backtest(df, symbol, tf, strategy, params, *, capital=1_000_000.0):
    ...
    if strategy in ("VWAP", "SMART_EMA"):
        ...
        rsi = calc_rsi(df["close"], params.get("rsi_period", 14))
        adx_val, plus_di, minus_di = calc_adx(df, params.get("adx_period", 14))
        macd_line, signal_line, histogram = calc_macd(df, params.get("macd_fast", 12), params.get("macd_slow", 26), params.get("macd_signal", 9))
        vwap = pd.Series(0, index=df.index)
    elif strategy == "BOLLINGER_REVERSION":
        bb_upper, bb_mid, bb_lower = calc_bollinger(df, params.get("bb_period", 20), params.get("bb_std", 2.0))
        rsi = calc_rsi(df["close"], params.get("rsi_period", 14))
        ema_fast = pd.Series(0, index=df.index)
        ema_slow = pd.Series(0, index=df.index)
        adx_val = pd.Series(0, index=df.index)
        plus_di = pd.Series(0, index=df.index)
        minus_di = pd.Series(0, index=df.index)
        vwap = pd.Series(0, index=df.index)
    elif strategy == "STRONG_TREND_WIDE":
        ema_fast = calc_ema(df["close"], params.get("ema_fast", 20))
        ema_slow = calc_ema(df["close"], params.get("ema_slow", 50))
        rsi = calc_rsi(df["close"], params.get("rsi_period", 14))
        adx_val, plus_di, minus_di = calc_adx(df, params.get("adx_period", 14))
        vwap = pd.Series(0, index=df.index)
...
def run():
    ...
    print("  " + "─" * 76)
    print("  WDO:")
    print("    M5  → SMART_EMA (pullback)     — EMA 9/21, ADX>25, RSI filter")
    print("    M15 → SMART_EMA (strong trend) — EMA 9/21, ADX>25, agressivo")
    print("    M30 → MACD_MOMENTUM            — MACD 12/26/9, ADX>15")
    print("    H1  → BOLLINGER_REVERSION      — BB 20/2σ, RSI<30/>70")
    print("  WIN:")
    print("    M5  → EMA_PULLBACK             — EMA 9/21, ADX>20, pullback")
    print("    M15 → EMA_PULLBACK             — mesmo com mais barras")
    print("    M30 → MACD_MOMENTUM            — MACD 12/26/9, ADX>15")
    ...
    combos = [
        ...
        ("WDO$", "M15", "SMART_EMA",          wdo_params),
        ("WDO$", "M30", "MACD_MOMENTUM",      macd_wdo_params),
        ("WDO$", "H1",  "BOLLINGER_REVERSION", bb_wdo_params),
        ("WIN$", "M5",  "EMA_PULLBACK",        win_params),
        ("WIN$", "M15", "EMA_PULLBACK",        win_params),
