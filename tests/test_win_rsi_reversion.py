...
class TestWinStrategyConsolidation(unittest.TestCase):
    ...
    def test_win_root_strategy_is_strong_trend(self):
        """strategy['WIN'] (root) deve ser STRONG_TREND (AGI evoluiu pós 19/06)."""
        from vt_config_loader import load_config
        config = load_config()
        self.assertEqual(
            config.get("strategy", {}).get("WIN"), "STRONG_TREND",
            f"WIN root deveria ser STRONG_TREND (AGI 867+), "
    ...
    def test_win_rsi_thresholds_realistic(self):
        ...
        self.assertGreaterEqual(rsi_os, 15, f"rsi_oversold={rsi_os} muito baixo")
