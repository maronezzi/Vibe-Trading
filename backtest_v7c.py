"""
backtest_v7c.py — Iteration 3 FINAL: STRONG_TREND for WIN, optimized VWAP for WDO.
"""
...
PARAMS = {
    "WIN": {
        ...
        "cooldown_seconds": 600, "max_daily_trades": 10,
        "trend_min_spread": 0.001,
    },
    "WDO": {
        "ema_fast": 9, "ema_slow": 21, "rsi_period": 14, "rsi_overbought": 85,
        "rsi_oversold": 15, "vwap_period": 20, "vwap_buy_threshold": 1.002,
        "vwap_sell_threshold": 0.998, "sl_atr_mult": 1.0, "trail_activate": 1.5,
        "trail_distance": 0.5, "cooldown_seconds": 300, "max_daily_trades": 8,
        "trend_min_spread": 0,
    },
}
...
def backtest_with_plugin(df, symbol, strategy_name, params, *, capital=100_000.0, tf_override="M5"):
    ...
    cash = capital
    pos = 0; ep = 0.0; e_date = None; e_atr = 0.0
    best = 0.0; sl_price = 0.0; trail_on = False; sl_pts = 0
    bars_in_trade = 0; last_trade_bar = -9999
    ...
    for i in range(n):
        ...
        for j in range(max(0, i - 59), i + 1):
            bar_list.append({
                "high": float(highs_arr[j]), "low": float(lows_arr[j]),
                "close": float(closes[j]), "open": float(opens[j]),
                "volume": int(vols[j]), "time": int(df.index[j].timestamp()),
            })
        ...
        if pos == 0:
            if cur_atr > 0 and (i - last_trade_bar) > 5:
                signal = check_entry(symbol, tf_override, price, cur_atr, str(bar_date), bar_list, params, utils)
                if signal:
                    ...
                    if cash >= margin + cost:
                        ...
                        best = price; trail_on = False; last_trade_bar = i; bars_in_trade = 0
                        sl_price = (price - sl_pts_raw) if pos == 1 else (price + sl_pts_raw)
            continue
    ...
    if pos != 0: _close(float(closes[-1]), "FORCE")
    ...
    for t in trade_log:
        r = t["reason"]
        if r not in pnl_by_reason: pnl_by_reason[r] = {"count": 0, "pnl": 0, "wins": 0}
...
def run():
    print("\n" + "═" * 100)
    print("  🧪 BACKTEST v7c — ITERATION 3 FINAL")
    print("  " + "─" * 96)
    print("  ✅ WIN: STRONG_TREND — aggressive trend-following, no RSI cap in strong trends")
    print("  ✅ WDO: VWAP with wider RSI (85/15) and tighter VWAP threshold (1.002/0.998)")
    print("═" * 100)

    combos = [
        ("WIN$", "M5", 500, "STRONG_TREND", "WIN"),
        ("WIN$", "M15", 500, "STRONG_TREND", "WIN"),
        ("WDO$", "M5", 500, "VWAP", "WDO"),
    ...
    for sym, tf, n_bars, strat_name, param_key in combos:
        ...
        r = backtest_with_plugin(df, sym, strat_name, params, tf_override=tf)
        if r and r["ok"]:
            print(f"\n  📊 {strat_name} — {sym} {tf}")
            print(f"  {'─' * 60}")
            ...
            all_results.append({"symbol": sym, "tf": tf, "strategy": strat_name, **r})

    if all_results:
        ranking = sorted(all_results, key=lambda x: x["ret"], reverse=True)
        print("\n\n" + "═" * 100)
        print("  📋 RANKING GERAL — v7c (FINAL)")
        print("═" * 100)
        ...
        print("\n\n" + "═" * 100)
        print("  📈 COMPARAÇÃO FINAL (v7c) vs BASELINE (v6)")
        print("═" * 100)

        baseline = {
            ("WIN$", "M5"): -0.36, ("WIN$", "M15"): -1.20,
            ("WDO$", "M5"): 0.22, ("WDO$", "M15"): 0.60,
        }

        print(f"\n{'Ativo':<10} {'TF':<4} {'v6 Ret%':>8} {'v7c Ret%':>9} {'Δ':>7}")
        print("─" * 50)
        total_delta = 0
        for r in all_results:
            ...
            print(f"  {r['symbol']:<7} {r['tf']:<4} {v6:>+7.2f}% {v7:>+8.2f}% {delta:>+6.2f}% {icon}")

        print(f"\n  📊 Delta total: {total_delta:+.2f}%")
