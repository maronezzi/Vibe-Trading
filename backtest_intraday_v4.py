"""
Backtest intraday v4 — saídas inteligentes + filtro ADX.

Mudanças vs v3:
1. Profit lock REMOVIDO de SMA/EMA (só RSI/BB/VWAP têm)
2. Trailing gatilho: 1.5x ATR (era 1x) — menos saídas prematuras
3. Filtro ADX: SMA/EMA só entram se ADX(14) > 25 (tendência forte)
4. Sinal inteligente: sai só se trailing ativo OU 2 barras neutras seguidas
5. Time-based: após 20 barras, trailing aperta (0.3x ATR)

Config:
  SMA/EMA → ADX>25 filtro + SL fixo (1.5x ATR) + Trailing (1.5x ATR) + sem profit lock
  RSI/BB/VWAP → Sem filtro ADX + só Trailing (1.5x ATR) + Profit lock (0.5x ATR)
Fecha tudo às 16:45 BRT.

Uso: PYTHONPATH=./agent ./agent/venv/bin/python backtest_intraday_v4.py
"""
...
def backtest(df, signals, symbol, *, capital=100_000.0, max_ct=3, use_sl=True,
    ...
    for i, (date, row) in enumerate(df.iterrows()):
        ...
        # === OTIMIZAÇÃO 2: SAÍDA POR SINAL INTELIGENTE ===
        if _is_exit_signal(target):
            # Só sai se: trailing já ativo OU 2 barras consecutivas neutras
            if trail_on or neutral_count >= 1:  # neutral_count já é 1 do bar anterior
                _close(price, "SIG")
            else:
                neutral_count += 1
                # Se acumulou 2 neutras seguidas, sai
                if neutral_count >= 2:
                    _close(price, "SIG")
                continue

        # Sinal continua favorável — zera contador
        if not _is_exit_signal(target):
            neutral_count = 0
...
def run():
    print("\n" + "═" * 120)
    print("  ⚡ BACKTEST INTRADAY v4 — SAÍDAS INTELIGENTES + FILTRO ADX")
    print("  " + "─" * 116)
    print("  ✅ Filtro ADX: SMA/EMA só entram se ADX(14) > 25 (tendência forte)")
    print("  ✅ Profit Lock: RSI/BB/VWAP — breakeven após 0.5x ATR (SMA/EMA sem)")
    print("  ✅ Trailing: gatilho 1.5x ATR (era 1x) — menos saídas prematuras")
    print("  ✅ Sinal inteligente: sai só se trailing ativo OU 2 barras neutras seguidas")
    print("  ✅ Time-based: trailing aperta (0.3x ATR) após 20 barras")
    print("  ✅ Config: SMA/EMA=ADX+SL+Trail(semBE) | RSI/BB=sóTrail+BE")
    ...
    if all_results:
        ...
        print("\n\n" + "═" * 140)
        print("  📋 RANKING GERAL — v3 SAÍDAS INTELIGENTES")
        print("═" * 140)
        ...
        # Comparação v2 vs v3
        print("\n\n" + "═" * 140)
        print("  📈 COMPARAÇÃO v2 (híbrido simples) vs v3 (saídas inteligentes)")
        print("═" * 140)

        v2_data = {
            "WIN$ M5": {"SMA(9,21)": -3.44, "RSI(7)": -0.75, "Bollinger": -0.22, "EMA(10,30)": -2.35, "VWAP": -0.28},
            "WIN$ M15": {"SMA(9,21)": -4.96, "RSI(7)": -2.24, "Bollinger": -0.00, "EMA(10,30)": -1.25, "VWAP": -0.82},
            "WDO$ M5": {"SMA(9,21)": -5.21, "RSI(7)": -5.28, "Bollinger": -1.27, "EMA(10,30)": -5.13, "VWAP": 0.07},
            "WDO$ M15": {"SMA(9,21)": -6.64, "RSI(7)": -0.39, "Bollinger": -0.15, "EMA(10,30)": -4.37, "VWAP": -3.02},
        }

        improvements = 0
        print(f"\n{'Ativo':<10} {'TF':<4} {'Estratégia':<14} {'v2 Ret%':>8} {'v3 Ret%':>8} {'Δ':>7}")
        print("─" * 60)
        for r in ranking:
            key = f"{r['symbol']} {r['tf']}"
            if key in v2_data and r["strategy"] in v2_data[key]:
                v2_ret = v2_data[key][r["strategy"]]
                v3_ret = r["ret"]
                delta = v3_ret - v2_ret
                icon = "✅" if delta > 0 else "❌" if delta < -0.5 else "➡️"
                print(f"  {r['symbol']:<7} {r['tf']:<4} {r['strategy']:<14} {v2_ret:>+7.2f}% {v3_ret:>+7.2f}% {delta:>+6.2f}% {icon}")
                if delta > 0:
                    improvements += 1

        total = sum(len(v) for v in v2_data.values())
